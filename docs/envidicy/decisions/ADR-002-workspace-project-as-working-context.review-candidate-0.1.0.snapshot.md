...
reviewers:
  ...
  - role: Operations/SRE Reviewer
    name: "Сухоруков Роман Николаевич"
    human_identity_ref: person:envidicy:0001
    required_decision_ids: [ADR-002-D02, ADR-002-D06, ADR-002-D07, ADR-002-D08, ADR-002-D09, ADR-002-D10]
    decision: null
    reviewed_at: null
    evidence_ref: null
...
Текущий статус — `REVIEW_CANDIDATE / PROPOSED`. Документ проектирует логические scopes внутри принятой tenant-модели ADR-001 и зафиксирован content-addressed manifest. Он ещё не находится в formal human review, не принимает Blueprint, не завершает H0.1 и не разрешает изменение application-code, database schema или production data.
...
- каждый tenant resource type заранее объявляет operational scope `ORGANIZATION`, `WORKSPACE` или `PROJECT`;
...
- исторический scope, authority и attribution после move/archive не переписываются.
...
|---|---|---|---|
| AP-001-R01/R02/R03/X01 | `I/C` | D01, D03, D05, D09, D10 | owners, projections и writers разведены |
...
| AP-005-R01–R03/R05 | `C` | D04, D05, D06, D08, D09 | owners меняют state через contracts; reparent intent идемпотентен |
| AP-008-R01/R02/R03/R05/X01 | `C` | D03, D04, D05, D06, D08, D09, D10 | default deny, least privilege и разные gates |
| AP-009-R01–R04/X01 | `I/C` | D06–D10 | scope changes и historical evidence доказуемы |
...
traceability_manifest:
  schema_version: 1
  canonical_set_id: ADR-002-TRACE-0.1.0
  merge_order:
...
- безопасные reparent/archive semantics;
...
- меняется одной audited operation без автоматического перемещения Projects/resources;
...
Универсальной shared `project_bindings` table с несколькими writers не создаётся. Canonical scope/association принадлежит authoritative owner самого resource или отдельному явно назначенному binding aggregate owner:
...
Каждый applicable ScopeRef/Grant разрешается против current `lifecycle_version` и `security_generation` base scope, каждого использованного ancestor-propagation edge и versioned overlay policy. Несовместимая либо неизвестная generation/version делает binding неэффективным до explicit revalidation.
...
`CONTROL_PLANE_SUPPORT` требует current scoped `ControlPlaneCaseGrant` и operator/workload grant. Для default switch, OrganizationMembership invitation/activation, grant/revoke, restore, reparent и другого действия, создающего или меняющего customer intent, consent, ownership, financial authority либо tenant permission, case grant сам не создаёт authority: дополнительно требуется отдельно tenant-authorized intent либо tenant-owner-authorized recovery/compensation decision.

Recovery/compensation evidence ссылается на ранее tenant-authorized intent или tenant-owned state, фиксирует exact digest/scope и не расширяет их. Approval platform/domain owner не заменяет tenant-owner authorization. Legal/compliance allowlist без tenant intent MAY разрешить только отдельно классифицированное обязательное действие и не создаёт либо не расширяет customer permission, ownership, financial authority, default selection или reparent scope.
...
4. сверяет client/job/event claims с current IDs и expected containment, lifecycle и security generations/versions;
5. определяет authority path ADR-001 (`DIRECT`, `DELEGATED`, `CONTROL_PLANE_SUPPORT`);
6. проверяет совместимость ScopeRef/Grant с current base/ancestor/overlay generations, затем permission, entitlement, feature flag, auth strength и approval как независимые gates;
...
- используется отдельная idempotent command/workflow с expected Project scope version;
- immutable preflight impact snapshot фиксирует версии Project/dependencies и before/after access delta;
- preflight impact не имеет unresolved `BLOCK`, а его версии не устарели к моменту commit;
...
Каждый dependency type заранее объявляет mobility policy:

| Policy | Семантика |
|---|---|
| `FOLLOW_PROJECT` | связь по stable `project_id` сохраняется, current Workspace выводится заново |
...
Unknown/missing mobility policy означает `BLOCK`.
...
- resource types обязаны заранее выбрать scope;
- UI «участники проекта» становится projection, а не простой membership table;
- same-Organization move требует dependency registry и preflight;
...
Project move использует собственный observation window и recovery plan. После появления material writes обратное действие не выполняется blind FK update; требуется новая authorizable move/compensation operation с impact validation.
...
|---|---|---|---|
| Core.Tenancy | Workspace/default lifecycle | Core Domain Owner | conformance после ADR acceptance |
| Core.Projects | Project identity/lifecycle/reparent | Core Domain Owner | conformance после ADR acceptance |
| Core.Authorization | ScopeRef, propagation, scoped grants | Core Domain Owner | ADR-005/006 contracts |
...
| Core Files | explicit tenant/operational scope | Core Files owner | scope classification |
...
| Advertising Media Planning | project-scoped plans | Advertising owner | classify/migrate legacy campaigns |
| Creative Intelligence | project-scoped sources/hypotheses | Creative owner | PRD/GF contract |
...
- control-plane case grant без independently authorized customer/recovery intent не выполняет reparent/restore/access change;
...
- Project move cross-Organization отклоняется;
- same-Organization move блокируется unknown mobility policy;
...
| Control-plane case пытается создать customer intent для reparent/restore/access change | `DENY` |
...
| Project move затрагивает unknown connection/financial binding | `BLOCK` |
...
- Project move, который не может безопасно revalidate dependencies;
...
Текущий `REVIEW_CANDIDATE / PROPOSED`:

- идентифицируется immutable snapshot и content-addressed manifest;
...
Formal `REVIEW` может быть открыт только для точного полного SHA-256 target через отдельные immutable review target и Review Pack. Любое изменение semantic content требует новой document version, нового snapshot и нового content-addressed manifest.
