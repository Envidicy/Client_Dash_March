...
Текущий статус — `REVIEW_CANDIDATE`; точный snapshot фиксируется связанным manifest. Решение остаётся `PROPOSED`: reviewer approvals не зафиксированы, `ACCEPTED` не установлен.

## 0. Краткое решение

Предлагается принять `Organization` как единственную корневую tenant boundary Envidicy.
...
## 16. Acceptance prerequisites

До `decision_status: ACCEPTED`:

1. ADR получает content-addressed Review Candidate snapshot.
2. Product/Business, Architecture, Core Domain, Security и Engineering фиксируют `APPROVE`.
3. Platform Domain, Operations/SRE, Finance и Data подтверждают указанные Decision IDs.
4. Нет unresolved contradiction с Constitution.
5. Нет незадокументированного `E`.
6. Candidate Matrix mapping нормализован до одной Rule–Decision связи на запись и проходит path/anchor/Decision ID validation.
7. Exact reviewed/accepted hashes, решения reviewers, даты и evidence refs фиксируются в ADR либо в предусмотренном текущим Governance review evidence; позднее они входят в финальный H0.1 Acceptance Manifest.
